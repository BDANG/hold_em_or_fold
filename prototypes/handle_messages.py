...
#global_data.USER_DICT = {'+11234567890':([0,1],0.0)} #customer dictionary. format "number":{"markets":[marketList],"minArgbitrage":float}
...
def handle_message(phoneNumber,message):
    if(message.lower() == "reset"): #user wants to select new markets
        print "Reset"
        #del global_data.USER_DICT[phoneNumber]
        global_data.USER_DICT[phoneNumber] = ([],0.0) #reset values
        send_message(phoneNumber,RESPONSE_DICT["newCust"])
        return

    elif(message.lower() == "stop"):
        print "Stop"
        del global_data.USER_DICT[phoneNumber] #remove customer from dictionary
        send_message(phoneNumber,RESPONSE_DICT["stop"])
        return

    elif(phoneNumber not in global_data.USER_DICT): #new customer
        print "New user"
        global_data.USER_DICT[phoneNumber] = ([],0.0)
        send_message(phoneNumber,RESPONSE_DICT["newCust"]) #new customer message
        return

    elif(global_data.USER_DICT[phoneNumber][0] == []): #user message contains markets to follow
        print "Get exchanges"
        exchanges = [] #temp list of exchanges from message
        valid = True
        for i in range(len(message)):
            if(int(message[i])-1 < 0 or int(message[i])-1 >= len(global_data.EXCHANGE_LIST)): #invalid number entry
                send_message(phoneNumber,RESPONSE_DICT["invalidNum"]) #let user know
                valid = False
                break
            else:
                exchanges.append(global_data.EXCHANGE_LIST[int(message[i])-1]) #add each exchange to list
        #user has sent valid message
        if(valid):
            global_data.USER_DICT[phoneNumber] = (exchanges,global_data.USER_DICT[phoneNumber][1])
            send_message(phoneNumber,RESPONSE_DICT["minArb"])
        return

    elif(global_data.USER_DICT[phoneNumber][1] == 0.0):
        print "get arbitrage"
        if(float(message)>100 or float(message<0)):
            send_message(phoneNumber,RESPONSE_DICT["invalidNum"])
        else:
            global_data.USER_DICT[phoneNumber] = (global_data.USER_DICT[phoneNumber][0],float(message)/100.0) #set customers min arb value
